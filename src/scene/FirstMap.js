import { MAP1_ASSET_KEYS, UI_ASSET_KEYS } from "../assets/AssetKeys";
...
import { MAP1_INTRO_TEXT, SAMPLE_TEXT } from "../utils/text-utils";
...
export class FirstMap extends Phaser.Scene {
    ...
    // 배경 등 형성하는 부분
    create(){
        console.log(`[${FirstMap.name}:create] invoked`);

        const x = 18.5 * TILE_SIZE;
        ...
        if (!collisionTiles){
            console.log(`[${FirstMap.name}:create] collision tileset error`);
            return;
        }
        const collisionLayer = map.createLayer('Collision', collisionTiles, 0, 0);
        if (!collisionLayer){
            console.log(`[${FirstMap.name}:create] collision layer error`);
            return;
        }
        ...
        this.add.image(0,0,MAP1_ASSET_KEYS.FM_FOREQROUND).setOrigin(0).setScale(4);
        ...
        const shown = dataManager.store.get(DATA_MANAGER_STORE_KEYS.MAP1_WELCOME_KEY);

        if (this._shouldShowIntro || !shown) {
            // 살짝 딜레이를 주면 페이드인 뒤 자연스럽게 뜸
            this.time.delayedCall(1100, () => this.#dialogUi.showDialogModal(MAP1_INTRO_TEXT));
              ...
              options: [
                {
                  ...
                  onSelect: () => {
                    ...
                    this.#dialogUi.hideDialogModal();
                    this.scene.pause(SCENE_KEYS.FIRST_MAP);
    
                    // 팝업 Scene 실행 (필요한 정보만 전달)
                    this.scene.launch(SCENE_KEYS.TALE_POPUP, {
                      mapId: 0,
                      taleId: npcTale.id,
                      returnSceneKey: SCENE_KEYS.FIRST_MAP,
                    });
                  }
                },
                {
  ...
  #getNpcTaleData(npc) {
    ...
    const taleData = TALE_INFO[0][npcIndex];
    console.log(`TALE_INFO에 ${npcIndex}번째 데이터가 있다.`);
    
    if (npcIndex < 0 || !TALE_INFO[0][npcIndex]) return null;
    ...
    npcLayers.forEach((layerName) => {
      ...
      null; // 없으면 NPC 쪽에서 기본값 씀
        ...
        const npc = new NPC({
            ...
            assetKey: npcSpriteKey, // ← spriteKey 값을 assetKey로 전달
          });
...
if (this.#npcs.length === 0) {
  ...
  #createExitAreas(map) {
    ...
    if (!exitLayer) {
      console.warn('[FirstMap] EXIT object layer 없음');
      return;
    }
    ...
    if (this.#exitAreas.length === 0) {
      console.warn('[FirstMap] EXIT 오브젝트가 하나도 없음');
    }
