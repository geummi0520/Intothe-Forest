import { MAP2_ASSET_KEYS, UI_ASSET_KEYS } from "../assets/AssetKeys";
...
import { MAP2_INTRO_TEXT, SAMPLE_TEXT } from "../utils/text-utils";
...
export class SecondMap extends Phaser.Scene {
    /** @type {Player} */
    #player;
    /** @type {Controls} */
    #controls;
    ...
    _handleResume(sys, data) {
      ...
      this.time.delayedCall(300, () => {
        ...
        this.add.image(2392,323,this.#lastCollected.img).setOrigin(0).setScale(4);
      });
    }
    ...
    // 배경 등 형성하는 부분
    create(){
        console.log(`[${SecondMap.name}:create] invoked`);
        ...
        const map = this.make.tilemap({ key: MAP2_ASSET_KEYS.SM_MAIN_LEVEL });
        const collisionTiles = map.addTilesetImage('collision', MAP2_ASSET_KEYS.SM_COLLISION);
        if (!collisionTiles){
        ...
        collisionLayer.setAlpha(TILED_COLLISION_LAYER_ALPHA).setOrigin(0).setDepth(2).setScale(4);
        ...
        this.add.image(0, 0, MAP2_ASSET_KEYS.SECOND_MAP).setOrigin(0).setScale(4);
        ...
        const MAP2_TALE_ID = 1;
        const alreadyCollected = inv.find(i => i.sourceTaleId === MAP2_TALE_ID);
        ...
        const shown = dataManager.store.get(DATA_MANAGER_STORE_KEYS.MAP2_WELCOME_KEY);

        if (this._shouldShowIntro || !shown) {
            // 살짝 딜레이를 주면 페이드인 뒤 자연스럽게 뜸
            this.time.delayedCall(1100, () => this.#dialogUi.showDialogModal(MAP2_INTRO_TEXT));
            dataManager.store.set(DATA_MANAGER_STORE_KEYS.MAP2_WELCOME_KEY, true);
        }
 ...
 update(time,delta) {
              ...
              options: [
                {
                  ...
                  onSelect: () => {
                    ...
                    this.#dialogUi.hideDialogModal();
                    this.scene.pause(SCENE_KEYS.SECOND_MAP);
    
                    // 팝업 Scene 실행 (필요한 정보만 전달)
                    this.scene.launch(SCENE_KEYS.TALE_POPUP, {
                      mapId: 1,
                      taleId: npcTale.id,
                      returnSceneKey: SCENE_KEYS.SECOND_MAP,
                    });
                  }
                },
                {
  ...
  #getNpcTaleData(npc) {
    ...
    const taleData = TALE_INFO[1][npcIndex];
    console.log(`TALE_INFO에 ${npcIndex}번째 데이터가 있다.`);

    if (npcIndex < 0 || !TALE_INFO[1][npcIndex]) return null;
...
if (this.#npcs.length === 0) {
  ...
  #createExitAreas(map) {
    ...
    if (!exitLayer) {
      console.warn('[SecondMap] EXIT object layer 없음');
      return;
    }
    ...
    if (this.#exitAreas.length === 0) {
      console.warn('[SecondMap] EXIT 오브젝트가 하나도 없음');
    }
  ...
  #goToNextMap() {
    ...
    this.cameras.main.once('camerafadeoutcomplete', () => {
        ...
        const spawnPosition = {
          x: 18  * TILE_SIZE, // 예시: 타일 32,1짜리 위치 같은 식으로 맞춰줘
          y: 20 * TILE_SIZE,
        };
    
        this.scene.start(SCENE_KEYS.THIRD_MAP, {
          intro: true,
          spawnPosition,
  ...
  #renderInventoryUI() {
    ...
    inventory.forEach((item, idx) => {
      ...
      const icon = this.add
        .image(startX + spacing * idx, startY, item.img)
        .setOrigin(0)
        .setScale(4);
