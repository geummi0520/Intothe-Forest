 ...
 * @property {import("../../common/direction").Direction} direction
 * @property {() => void} [spriteGridMovementFinishedCallback]
 * @property {CharacterIdleFrameConfig} idleFrameConfig
 * @property {Phaser.Tilemaps.TilemapLayer | undefined} [collisionLayer]
 */
...
export class Character {
  ...
  _phaserGameObject;
  /** @protected @type {import("../../common/direction").Direction} */
  _direction;
  ...
  _isBlockingTile(){
    ...
    const updatedPosition = getTargetPositionFromGameObject(targetPosition, this._direction);
    // console.log(`Current Pos: (${targetPosition.x}, ${targetPosition.y}), Target Pos: (${updatedPosition.x}, ${updatedPosition.y})`);
  ...
  #handleSpriteMovement(){
    ...
    const updatedPosition = getTargetPositionFromGameObject(this._targetPosition, this._direction);
  ...
  #doesPositionCollideWithCollisionLayer(position){
    ...
    const { x, y } = position;
    const scale = 4;
    const charWidth = 16 * scale;  // 64
    const charHeight = 32 * scale; // 128
    const scaledTileSize = TILE_SIZE; // 64

    // Y 축: 캐릭터 발 기준으로 보정 (발끝 바로 위)
    const correctedY = y + charHeight - 1; 

    // X 축: 캐릭터 중심으로 보정
    const correctedX = x + charWidth / 2; 

    // 타일 좌표는 바닥과 중심 기준으로 floor 로 통일
    const tileX = Math.floor(correctedX / scaledTileSize);
    const tileY = Math.floor(correctedY / scaledTileSize);

    const tile = this._collisionLayer.getTileAt(tileX, tileY, true);
